...
import { useProfessionalisation } from '@/hooks/useProfessionalisation'
import ProfessionalisationPanel from '@/components/ProfessionalisationPanel'
...
export default function Dashboard() {
  ...
  const { balance, hasFreeScan, hasPaidCredits, purchase, refresh: refreshCredits } = useCredits()
  const [searchParams, setSearchParams] = useSearchParams()
  const [purchasing, setPurchasing] = useState<CreditTier | null>(null)
  const [purchaseError, setPurchaseError] = useState<string | null>(null)

  // Professionalisation: use the first project's ID, or null if no projects
  const activeProjectId = projects.length > 0 ? projects[0].id : undefined
  const {
    progress: profProgress, guide: profGuide, loading: profLoading,
    initProgress, markExported, markGithubStored, markEnvExampleDone,
    markBuildWorks, markSecurityFixed,
  } = useProfessionalisation(activeProjectId)
  ...
  }, [checkoutStatus, refreshCredits, searchParams, setSearchParams])
    ...
    <div className="min-h-screen px-4 py-12">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        {checkoutStatus === 'success' && (
          <div className="border border-green-500/50 p-4 text-sm text-green-400">
            PAYMENT SUCCESSFUL. YOUR CREDITS HAVE BEEN ADDED.
          </div>
        )}
        {checkoutStatus === 'cancelled' && (
          <div className="border border-yellow-500/50 p-4 text-sm text-yellow-400">
            CHECKOUT CANCELLED. YOUR CREDITS REMAIN UNCHANGED.
          </div>
        )}
        {purchaseError && (
          <div className="border border-destructive/50 p-4 text-sm text-destructive">
            {purchaseError}
          </div>
        )}
        ...
        <div className="report-card">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="space-y-1">
              <p className="text-xs text-muted-foreground tracking-wider">CREDIT BALANCE</p>
              <div className="flex items-center gap-6 text-sm">
                <span className={hasFreeScan ? 'text-green-400' : 'text-muted-foreground'}>
                  FREE SCANS: {balance.freeScans}
                </span>
                <span className={hasPaidCredits ? 'text-green-400' : 'text-muted-foreground'}>
                  PAID CREDITS: {balance.paidCredits}
                </span>
              </div>
            </div>

            {/* Quick-buy if no credits */}
        ...
        {/* ── Professionalisation Panel ── */}
        {projects.length > 0 && (
          <ProfessionalisationPanel
            progress={profProgress}
            guide={profGuide}
            loading={profLoading}
            onInit={initProgress}
            onMarkExported={markExported}
            onMarkGithubStored={markGithubStored}
            onMarkEnvExampleDone={markEnvExampleDone}
            onMarkBuildWorks={markBuildWorks}
            onMarkSecurityFixed={markSecurityFixed}
          />
        )}
        ...
        <div className="space-y-4 text-sm">
          ...
          <ul className="space-y-2 text-muted-foreground">
            ...
            <li>▪ <Link to="/architect" className="hover:text-foreground transition-colors">Plan a new idea with the Idea Architect</Link></li>
